...
var test_str = 'နေကောင်းရဲ့လား';
var offset = 0;
var result = [];

init();
while(offset < test_str.length) {
	var type_n_next = get_next_syllable(test_str, test_str.length, offset);
	var type = type_n_next[0];
	var next = type_n_next[1];

	result.push(test_str.substring(offset, next));
	offset = next;
}
